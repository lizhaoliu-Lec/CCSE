...
class SparseRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # Build Dynamic Head.
        self.head = DynamicHead(cfg=cfg, roi_input_shape=self.input_shape)
    ...
    def forward(self, batched_inputs):
        ...
        # Prediction. for box and class only
    ...
    def preprocess_image(self, batched_inputs, prefix=None):
        ...
        """
        if prefix is not None:
            images = [self.normalizer(x["{}_image".format(prefix)].to(self.device)) for x in batched_inputs]
        else:
